...
public class BusyDaysCommandHandler implements CommandHandler {
    ...
    public BusyDaysCommandHandler(OpenCommandHandler openCommandHandler) {
    ...
    public void handleCommand(String command) {
        if (command.startsWith("busydays")) {
            if (openCommandHandler.isFileOpened()) {
                // Имплементация за извеждане на статистика за натовареност
                System.out.println("Статистика за натовареност:");
                // Изведете списък с дните от седмицата, подредени по критерия "брой заети часове"
            } else {
